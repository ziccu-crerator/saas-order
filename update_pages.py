...
def generate_sidebar(active_page_filename):
    ...
    core_pages = PAGES[:6]
    finance_pages = PAGES[6:10]
    settings_pages = PAGES[10:]
    ...
    def render_links(pages):
        res = ""
        for p in pages:
            active_class = " active" if p[0] == active_page_filename else ""
            res += f'''
                <a href="{p[0]}" class="app-nav-link{active_class}">
                    <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">{icons[p[0]]}</svg>
                    {p[1]}
                </a>'''
        return res

    sidebar += render_links(core_pages)
    sidebar += '''\n\n                <div class="app-nav-label">Finance & Operations</div>'''
    sidebar += render_links(finance_pages)
    sidebar += '''\n\n                <div class="app-nav-label">Settings</div>'''
    sidebar += render_links(settings_pages)
    
    sidebar += '''
            </nav>

            <div class="app-sidebar__footer">
                <a href="login.html" class="app-nav-link">
...
def generate_topbar(title):
    return f'''            <header class="app-topbar">
                ...
                <div class="app-topbar__actions">
                    <div class="app-topbar__search">
                        ...
                        <input type="text" placeholder="Search orders, vendors, customers...">
                    </div>
...
def create_shell(filename, title):
    content = base_head.replace("{title}", title) + "\n" + generate_sidebar(filename) + '\n        <main class="app-main">\n' + generate_topbar(title) + f'''
            <div class="app-content">
                <div class="app-card">
                    ...
                    <div class="app-card__body">
                        <p style="color: var(--neutral-600);">This module is currently undergoing an enterprise platform update. Please check back later.</p>
                    </div>
                </div>
            </div>
        </main>''' + base_foot
...
for p in PAGES:
    if p[0] != "dashboard.html": # Already generated dashboard.html earlier
        if not os.path.exists(p[0]) or p[0] in ["invoices.html", "reports.html", "notifications.html", "support.html", "settings.html"]:
            create_shell(p[0], p[1])
